...
> **【補足】このノートの役割**
> 設計書5種（[設計書 システム構成](design/system.md) / [設計書 インタフェース](design/interface.md) / [設計書 データベース](design/database.md) / [設計書 画面遷移図](design/screens.md) / [設計書 プロンプト](design/prompt.md)）が持つのは**「何を作るか」**。
...
> 括ると、後者だけが本番で牙を剥く。** 2 を直したのは、第三者テスト——
...
### 4-10. 実物と突き合わせて初めて見つかった記述の誤り（2026-09-01 夕）

**リポジトリ直下に `README.md` を書くために、全記述を実物と1つずつ照合した。そこで5件出た。**
コードは1バイトも触っていない——**間違っていたのは記述のほうで、実装と設計判断は正しかった。**

| # | どこ | 書いてあったこと | 実物 | 確かめる先 |
|---|---|---|---|---|
| 1 | [ADR-0005 開発環境](adr/0005-dev-env.md) §3-4 | edition 2024 に **1.85 以上** | **1.95 以上。** `topcoat` と `toasty` が `rust-version = "1.95"` を宣言しており、1.85〜1.94 は依存解決で弾かれる | `Cargo.lock` |
| 2 | [ADR-0001 技術選定](adr/0001-tech-stack.md) §2・§3-3 ／ [スコープと縮退ライン](scope.md) §6 | **Topcoat は Axum の上に構築されている** | `Cargo.lock` に `axum` は **0件**。`topcoat-router` は `hyper` / `hyper-util` に直接依存 | `Cargo.lock` |
| 3 | [設計書 インタフェース](design/interface.md) §3-1 | 静的ファイルは `GET /assets/*` | **そのパスは存在しない。** `asset!()` が `/_topcoat/assets/<名前>-<内容ハッシュ>.<拡張子>` を生成し、ファイル1つにつき1ルートを自動登録する | `src/ui.rs` ／ `src/pages/*.rs` |
| 4 | [テスト項目書](test.md) §6 | 現在 **77項目** | **79項目**（§5 の集計も層別表も79） | **同じ文書の §5** |
| 5 | [テスト項目書](test.md) §3-6 | shim は **v24.15.0** | **v22.12.0。** nodenv に入っているのはこの1つだけで、`which node` も同じものを返す | `nodenv versions` |

**5件とも Vault 側で直した。** 『基本設計書』 は 2026-09-01 に提出済みなので、**#2（第1章）と #3（第4章）は提出物とずれる。**
再提出しないと決めたのは、**正典は 9/15 まで読まれ続け、提出済みファイルはもう誰も参照しないため**である。
正典は `tools/build_docs.py` で `docs/` へ降り、WSL 側のセッションが読む。**古いまま残すほうが高くつく。**

#### 5件は原因を共有していない

「ドキュメントが実装に追いついていない」という単一の説明は成立しない。**書かれた時期がばらばらである。**

| 書かれた日 | 実装着手（8/31）との前後 | 該当 |
|---|---|---|
| 8/26 | **5日前** | #2 |
| 8/30 | **前日** | #1 |
| 8/31 | **当日・実装と並行** | #5 |
| 9/1 09:51 | **実装完了後**（項番78・79 を足したとき §6 だけ追随せず） | #4 |
| 9/1 | **実装完了後。実装から書き起こした章** | #3 |

**#3 が反例として最も重い。** [設計書 インタフェース](design/interface.md) は「すでに動いている実装から書き起こした」章であり、
**コードを見ながら書いて、なお存在しないパスを書いた。**「実装より前に書いたから間違えた」では説明できない。

共通しているのは原因ではなく、**どれも確かめれば5分で分かることを、確かめずに書いた**という一点だけである。
確かめる先が外（`Cargo.lock`・実際のルーティング・`nodenv versions`）か内（同じ文書の §5）かが違う。

> **【重要】検査を5系統作って、5件とも1本も引っかからなかった**
> `cargo test` 26件・`check/*.mjs` 14件・`check/httpcheck.py` 26件・`build_*.py` のハッシュ照合・
> `tools/check_links.py`——**記述の真偽を見る検査は、この中に1本も無い。**
> `check_links.py` はリンクの**実在**を見るが**真偽**は見ない。ハッシュ照合は生成物の手編集を見るが
> **出典の正しさ**は見ない。
>
> **引っかけたのは検査ではなく、「外部の読者に向けて1枚書く」という作業だった。**
> `README.md` は、全記述を実物と通しで突き合わせることを初めて要求した成果物である。
>
> §4-9 の「**検査は書いた場所しか守らない。書くために読むことのほうが利いている**」が、
> 読む対象をコードから記述へ移して、もう一度成立した。
...
> **実装で直すものではなく、運用で回避する。**
> 9/15 の発表と、その直後の第三者テストでは、**渡す前に1本流して温めておく。**
...
#### 2026-09-01：L4 を実際に走らせた（項番65〜69）

**4問中1問が Yes/No 型で、1問が下限割れ。** 台本回答で fog の長短分岐を両方通した（→[テスト項目書](test.md) §4-12）。

| turn | 直前回答 | steer | 問い | 字数 | 判定 |
|---|---|---|---|---|---|
| 1 | 27字 | 一歩ずつ深める。 | 引き受けてしまったことで、今どんなことが起きていますか。 | **28** | **下限割れ** |
| 2 | 14字 | 深めない。同じ深さで聞き直す。 | 断ったら気まずくなる気がした。その気まずさは、どんな場面を想像していたんですか。 | 40 | 異常なし |
| 3 | 43字 | 一歩ずつ深める。 | 相手が忙しそうにしていて、仕事が回らなくなると思った。その時、自分が引き受けないという選択肢は、どう見えていたんですか。 | **60** | 異常なし |
| 4 | 12字 | 深めない。同じ深さで聞き直す。 | 嫌われたくなかった。相手に嫌われるのと、引き受けることの間に、何か迷いがあったんですか。 | 44 | **Yes/No 型** |

**2件とも、なぜそうなったかまで特定できた。**

**Yes/No 型（turn4）は、few-shot 例2 の構造を写して疑問詞だけ落としたもの。**
例2 は「店長のことと、店が回らないこと。最終的に辞める側に傾いたのは、**どちらが**変わったからですか。」で、
turn4 は前半の「2要素を並べる」構造をそのまま写しながら、**末尾を「〜んですか」で閉じて疑問詞を落としている。**
並べる形自体は例2 が**良い例として与えている**ので、禁止すると例2 と矛盾する。
なお二択判定は正しく素通りさせている——turn4 は2要素を並べるが**選ばせていない**。
8/31 に入れた判定が誤検知しないことの裏が、ここで取れた。

**下限割れ（turn1）は、正典が上限にしか行動指示を持たないことによる。**
正典の1行目は `日本語で30〜60字。**60字を超えたら削る**` で、**範囲は両端を書いているのに、行動は上限側にしか無い。**
結果が綺麗に割れている——**turn3 はちょうど60字**（指示のある上限に張り付いた）、**turn1 は28字**（指示の無い下限を割った）。
**指示のある側だけが守られている。**

> **【メモ】直さないと決めた（2026-09-01）**
> 下限側にも行動指示を足せば直る見込みが高いが、**入れていない。**
> [テスト項目書](test.md) §2 のとおり不合格をゼロにすることは目標ではなく、
> 定期試験の評価対象は内省である（→『2学期定期試験 項目』）。
> **「規則を書けば守られる」わけではなく、行動として書いた側だけが守られた**という対照そのものが記録として価値が高い。

**課金の見積りも外れていた。** [テスト項目書](test.md) 項番65 の備考は「1本 $0.0014 前後」だったが、**実測 $0.00623**（4.4倍）。
**毎ターン過去のやりとりを全部送り直す**ので `input_tokens` が累積する（1問目1,225 → 4問で5,384）。
サーバは会話の途中状態を持たない設計（→[設計書 システム構成](design/system.md) §3(a)）の、そのまま裏返しのコストである。

**本当の検査は 9/15 発表後の第三者テスト**（2026-09-01 に測定時点を移した。→[第三者テスト台本 20260915](usertest.md)）。
ただし**発表後に見つけても直す機会が無い**——プロンプトの文言はフリーズ後も直してよいが（→§9）、その日には間に合わない。
...
| **問いの品質** | 2026-09-01 の L4 実測で**4問中1問が Yes/No 型・1問が下限割れ**（→§6）。原因は特定済みだが**直さないと決めた**。机上の検査はここで打ち切りで、**本当の検査は 9/15 発表後**。**そこで見つけても直す機会は無い** |
| **SSE の失敗が成功に化ける経路** | Anthropic がストリームの途中で `error` フレームを送ると、`drain_frames` の `_ => {}` が捨て、`failed` を出さずに `done` が出る。**切れた問いが完成扱いで残る**（→[設計書 インタフェース](design/interface.md) §3-3 の③）。**2026-09-01 に、実装は触らず記録だけ残すと決めた**——①②は画面として成立しており、③は Anthropic 側が落ちたときにだけ起きる。**「起きない」のではなく「起きても気づけない」** |
| **第三者テストの測定条件** | **9/9〜9/10 にテスターを確保できず、9/15 の発表後へ移した**（→[スコープと縮退ライン](scope.md) §3）。定義①の判定（ノードが増えたか）は測れるが、**聴衆は発表を聞いているので「初見の入口の軽さ」は測れない。測定条件を自分で下げた**（→[第三者テスト台本 20260915](usertest.md) §3） |
| **発表後に見つかる不具合** | 上記の帰結。**直す機会が構造的に無い。** 9/9〜9/10 案は「フリーズ直後に置いて、壊れた箇所を直す時間を残す」設計だった（→[ドキュメントハブ](status.md) の日割り）。**その余地ごと失っている** |
...
- 1画面に入る会話数：**23本 → 84本**（実測）。**シードした30会話**でも全点が収まる（**実データは会話11本**。→§6）
...
- [ ] 第三者3人以上に触ってもらう（**9/15 発表後**。2026-09-01 に 9/9〜9/10 から移した）——**これが成功の定義そのもの**
- [x] レポート04〜06 ← **残り日程から見て最大のリスクだった**（2026-09-01 完了）
- [x] プレゼン資料（提出物⑪）→ **14枚（➕2枚込み）をドライブへ引き渡し済み**（2026-09-09）
