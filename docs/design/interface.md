...
> 他の設計書が「**実装前に決めたこと**」の記録であるのに対し、本書は 2026-09-01 に、すでに動いている実装から書き起こした。
> 代わりに、[設計書 システム構成](system.md) §4 が持っていた**旧設計との差分を §7 に置く**。
>
> **当初ここには「したがって設計と実装の食い違いを持たない」と書いていた。同日夕に反例が1件出たので取り下げる**——
> §3-1 の静的ファイルのパスが実在しないものだった（→[詳細設計書](../detail.md) §4-10）。
> **実装を見ながら書くことは、正確さを保証しない。**
...
| `GET` | `/_topcoat/assets/<名前>-<内容ハッシュ>.<拡張子>` | — | 静的ファイル | `script.js` / CSS。**素のまま配る**（→[ADR-0001 技術選定](../adr/0001-tech-stack.md)）。パスは `asset!()` が生成し、**ファイル1つにつき1ルートを自動登録する**——ワイルドカードではない（2026-09-01 訂正。→[詳細設計書](../detail.md) §4-10） |
...
| `failed` | エラーの文言 | **受信中に落ちた。** ブラウザは `EventSource.close()` して「うまく問いを作れませんでした」へ（→[設計書 画面遷移図](screens.md) §5-1） |
| `done` | — | 生成の完了。**これが無いと「完了」と「失敗」を区別できない** |

> **【重要】`done` は「成功」を意味しない**
> 見るべきは「`done` が来たか」ではなく「**末尾が** `done` か・`failed` が来なかったか」である。
> 実装は `deltas.chain(done)` なので、**`failed` を出したあとにも `done` が続く**。
> ブラウザ側は `failed` のハンドラで `close()` するため `done` は届かず**画面は成立している**が、
> プロトコルとしては「`done` が来た＝成功」が成り立っていない。
> [テスト項目書](../test.md) 項番36 の検査もこれに合わせ、`delta` が1件以上・**末尾が `done`**・`failed` が無い、の3つを見る。
...
| 受信中にブラウザが閉じた | — | Anthropic 側のストリームも落とす（→[設計書 プロンプト](prompt.md) §12 で実測。2026-09-01 の項番36 でも `[sse ] ストリームが drop された` を確認） |
| **生成の途中で失敗した** | **`failed` イベント（②）／ 何も出ない（③）** | 下記「失敗したときに何が出るか」 |

#### 失敗したときに何が出るか（2026-09-01 に確定）

**旧版はここを「未確認・未決定」としていたが、決まっていないのではなく、決めて実装したものが本書へ書き戻されていなかった。**
[テスト項目書](../test.md) 項番36 の実施（2026-09-01）とコードの読み合わせで、**失敗は1つではなく3経路**あることが分かった。

| # | どこで失敗するか | 出るもの | 画面 |
|---|---|---|---|
| ① | **SSE を始める前**（`ANTHROPIC_API_KEY` が無い／Anthropic が 4xx・5xx を返す） | **SSE ではなく HTTP 500**。`from_env()?` と `ask().await?` はストリーム生成の前にある | `onerror`（`got == 0`）→ 失敗表示 |
| ② | **受信中**（TCP／`reqwest` レベルで切れた） | **`failed` イベント**。その後 `chain` により `done` も続く | `failed` を購読して失敗表示 |
| ③ | **Anthropic がストリームの途中で `error` フレームを送った** | **何も出ない。`done` だけが出る** | **完了として扱われ、途中までの問いが残る** |

> **【不具合】③ が穴。失敗が成功に化ける**
> `drain_frames` の分岐は `message_start` ／ `content_block_delta` ／ `message_delta` の3つだけで、
> **残りは `_ => {}` で捨てている。** Anthropic の `{"type":"error"}`（`overloaded_error` など）はここに落ちて消える。
> 接続が閉じると `inner.next()` が `None` を返し、`chain(done)` が `done` を出す——
> **切れた問いが、完成した問いとして画面に残る。** `stop_reason: "max_tokens"` も同じ層で握り潰されるが、
> 問いは30〜60字で `max_tokens` は300なので実害は薄い。
>
> **2026-09-01：実装は触らないと決めた**（→[詳細設計書](../detail.md) §9）。9/8 フリーズまでの残り日数に対して、
> ①②が画面として成立しており、③は Anthropic 側が落ちたときにだけ起きる。
> **ただし「起きない」のではなく「起きても気づけない」**ので、直さないことと知らないことは別物として記録しておく。
