...
python3 check/http.py                         L3  HTTP＋DB の不変条件（新設）
...
| **L3** | `POST /talk/answer` / `GET /talk/question` / `/map` / `/` / 木構造 | `python3 check/http.py` | 25〜50 | **無い。新設** |
...
| L3 | **`check/http.py`（新設）** | 下記 |
| L4 | `check/run.py`（既存。触らない） | — |
...
1. **失敗したら終了コード1。** 既存3本と同じ形（`if (fails.length) { console.error(...); process.exit(1); }`）。人が目視で `✅` を数える形にしない
